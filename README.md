...
| **Market State** | `GET /v1/market/session`<br>`GET /v1/market/order-queue/{symbol}`<br>`GET /v1/market/intraday/{symbol}?kind=price\|brokers` | Authoritative IDX session state, resting order queue depth, and minute-resolution session tape + intraday broker flow timeline (served full-session, even after close). |
| **Indexes & Peers** | `GET /v1/indexes/{index_code}/members`<br>`GET /v1/fundamentals/{symbol}/peers` | Index constituent baskets (LQ45, IDX30…) with inline last prices; peer-relative multiples vs subsector industry aggregate. |
| **Corp Status** | `GET /v1/market/corpaction-status?symbols=A,B,C`<br>`GET /v1/calendars/day/{day}`<br>`GET /v1/market/earnings`<br>`GET /v1/underwriters/{code}/performance` | Batch UMA/special-notation flags, market-wide day calendar buckets, market-wide earnings recap (consensus vs actual), and per-underwriter IPO track records. |
...
| **Analytics** | `GET /v1/analytics/vwap/{symbol}`<br>`GET /v1/analytics/flow/{symbol}` | SQLite-aggregate VWAP, buyer/seller flow volume imbalance ratio over stored ticks (DuckDB/Parquet removed — no downstream consumer). |
...
- [x] **Daily Candles Date-Order Quirk & Multi-Year Slicing (`/chartbit/{symbol}/price/daily`)**:
  - *Elevated Implementation*: Swapped parameters handled by `build_daily_params`. Activated parallel multi-year window slicing across `SLICE_DAILY` (365d) bounds with `asyncio.gather` (throttled once, inside `HttpxTransport`), reducing multi-year query wall-clock latency from $O(N \times \text{RTT})$ to $O(\text{RTT})$.
...
- [x] **Wire-Speed Zero-Allocation Ingestion & Pre-Serialization (`ingest-rs` + `Hub`)**:
  - *Elevated Implementation*: Introduced thread-local decompression scratch buffers (`decompress_into`) in `ingest-rs`, dropping raw deflate decompression p95 tail latency by **81.7%** (11.4 µs vs 62.2 µs). Pre-serialized JSON text in `Hub.publish()` and fast-pathed `q.full()` queue drops, reducing 100-client fanout latency by **52.5%** and 500-client fanout latency by **28.1%**. 2026-09-04: added `Hub.publish_batch()` fast path (single client pass per batch, bulk drop-oldest eviction) and batched embedded `running_trade_batch` fan-out; 500-client throughput rose 4.3k → 6.4k ev/s (+29% per-event). Production output is Redis Streams only (direct-IPC TCP transport removed — it capped the feed at one consumer).
- [x] **SQLite VWAP/Flow Analytics (`infra/archive.py`)**:
  - *Current*: VWAP + buyer/seller flow imbalance via one SQLite aggregate over the tick store (`asyncio.to_thread`). DuckDB + Parquet export removed (no downstream consumer; ~30MB dep dropped).
...
- [x] **Upstream Route Expansion (2026-09-04)**:
  - *Elevated Implementation*: Added 10 verified upstream routes into `StockbitProvider` + REST: `/company-price-feed/market-time/session` (`GET /v1/market/session`), `/order-trade/running-trade/chart/:symbol` (`GET /v1/market/intraday/{symbol}?kind=price|brokers`), `/order-trade/order-queue` (`GET /v1/market/order-queue/{symbol}`, key is `stock_code`), `/emitten/indexes/:indexCode` (`GET /v1/indexes/{code}/members`), `/comparison/:symbol/ratios|industries` (`GET /v1/fundamentals/{symbol}/peers`), `/corpaction/status` (`GET /v1/market/corpaction-status`), `/corpaction?date=` (`GET /v1/calendars/day/{day}`), `/order-trade/underwriters/:code/ipo-performance` (`GET /v1/underwriters/{code}/performance`). Bare `/order-trade/underwriters` directory confirmed 404 upstream — per-code route only. `/earnings` vocabulary settled live 2026-09-05 (`sort_column` + `order` required, `filter` rejected) — full 15/15 live verification green.
  - *Wire-Quirk Fixes*: `trade_book` now sends numeric `group_by="1"` (legacy `GROUP_BY_PRICE` normalized in provider); `broker_summary` accepts native `period` windows (YTD/7D/3M) with silent `period`-vs-`from`/`to` exclusivity enforced client-side; `broker_summary`/`brokers_top`/`top-stocks`/`broker_activity` filter params (transaction_type, boards, sort, order, value_type, page, limit) now forwarded instead of silently dropped; `broker_distribution` accepts explicit `from`/`to` with period/date exclusivity.
