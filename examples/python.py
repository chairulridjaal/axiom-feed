...
with httpx.Client(timeout=30) as c:
    ...
    with c.stream(
        "GET",
        f"{BASE}/v1/candles/BBCA",
        params={"from": "2026-08-01", "to": "2026-08-26", "resolution": "daily"},
    ) as r:
        r.raise_for_status()
    ...
    print(
        "health:",
        c.get(
            f"{BASE}/v1/health", headers={"X-API-Key": API_KEY} if API_KEY else {}
        ).json(),
    )
    print(
        "quote:",
        c.get(
            f"{BASE}/v1/quotes/BBCA", headers={"X-API-Key": API_KEY} if API_KEY else {}
        ).json(),
    )
    print(
        "book:",
        c.get(
            f"{BASE}/v1/books/BBCA", headers={"X-API-Key": API_KEY} if API_KEY else {}
        ).json(),
    )
...
async def ws_demo():
    uri = (
        f"ws://localhost:8000/v1/stream?token={API_KEY}"
        if API_KEY
        else "ws://localhost:8000/v1/stream"
    )
    async with websockets.connect(uri) as ws:
        ...
        await ws.send(
            json.dumps({"action": "subscribe", "symbols": ["*"], "kinds": ["quotes"]})
        )
